...
def clean_train_data(dataset: str, device: str, batch_size: int) -> tuple[DataLoader, Any, Any]:
    ...
    file_data = read_npz_files(f"Data/MultiNLI/{dataset}_embeddings.npz")  # Read embeddings of data generated by BERT
    # Create training and validation sets
    x_data, y_data = [], []

    for data in file_data:
        x_data.append(data)
        y_data.append(labels[len(x_data) - 1])

    # Convert x_data and y_data to tensors
    x_data: torch.Tensor = torch.tensor(x_data).to(device).reshape(len(x_data), data.shape[0], data.shape[1])
    y_data: torch.Tensor = torch.tensor(y_data).to(device)

    # Split data into training and validation sets
    x_train, x_val, y_train, y_val = train_test_split(x_data, y_data, test_size=0.2, random_state=42)

    # Convert training and validation data and labels to TensorDataset
    train_dataset: TensorDataset = TensorDataset(x_train, y_train)

    # Create DataLoaders with shuffled data
