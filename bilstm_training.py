...
def clean_train_data(dataset: str, device: str, batch_size: int) -> tuple[DataLoader, Any, Any]:
    ...
    for data in file_data:
        ...
        y_data_list.append(labels[len(x_data_list) - 1])

    num_samples = len(x_data_list)

    x_tensors = []
    y_tensors = []

    for i in range(0, num_samples, batch_size):
        x_batch = x_data_list[i:i+batch_size]
        y_batch = y_data_list[i:i+batch_size]
