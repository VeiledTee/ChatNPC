...
TO_CLEAN: str = "Data/contradiction-dataset.csv"

df = pd.read_csv(TO_CLEAN)
df["label"] = np.where(df["gold_label"] == "contradiction", 1, 0)
...
df.to_csv(f"{TO_CLEAN[:-4]}_cleaned.csv")
